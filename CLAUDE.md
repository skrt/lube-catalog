...
| # | セクション | 出所 | 点灯条件 |
|---|---|---|---|
| 1 | Usage | components.json `usage`（when / avoid / vs / examples） | usage が存在 |
...
| 入ってくるもの | 行き先 | 実体 |
|---|---|---|
| 用途・使い分けのルール（いつ使うか / 使わないケース / 他コンポーネントとの使い分け / 実例） | Usage | components.json `usage`（`when` / `avoid` / `vs` / `examples` の任意キー・各 string 配列） |
